...
namespace Project.Web.Base.Utility
{
	...
	public class ExpressionCondition<T>
	{
		...
		/// </summary>
		public ExpressionRelation PrefixConditionRelation
		{
			get;
			set;
		}
		...
		{
			if (value == null && valueRelation != ExpressionValueRelation.NotEqualNull && valueRelation != ExpressionValueRelation.EqualNull)
				return this;
			...
			try
			{
				if (valueRelation != ExpressionValueRelation.Contains && valueRelation != ExpressionValueRelation.NotEqualNull && valueRelation != ExpressionValueRelation.EqualNull)
				{
			...
			switch (valueRelation)
			{
				...
				case ExpressionValueRelation.Contains:
					...
					if (value.GetType().BaseType == typeof(Array))
					{
						containMethod = typeof(Enumerable).GetMethods(BindingFlags.Static | BindingFlags.Public).First(mi => mi.Name == "Contains").MakeGenericMethod(memberExpr.Type);
		...
		private ExpressionCondition<T> ComposeCriteria(Expression<Func<T, bool>> criteria, ExpressionRelation expressionRelation)
		{
			...
			invocationExpressionOne = Expression.Invoke(Criteria, parameter);

			InvocationExpression invocationExpressionTwo = null;
			invocationExpressionTwo = Expression.Invoke(criteria, parameter);
			...
			Criteria = Expression.Lambda<Func<T, bool>>(composedCondtion, parameter);
			return this;
		}
		...
		private ExpressionCondition<T> ComposeEnumCondition<U>(Expression<Func<T, object>> member, object value, ExpressionValueRelation valueRelation, ExpressionRelation expressionRelation, EnumPart partForShow,EnumPart partForSave)
		{
			if (valueRelation == ExpressionValueRelation.Contains)
			...
			//定义object 类型数组
			object list = null;
			MethodInfo helperMethod = typeof(ReflectionTools).GetMethod("BuildListHelper");
					...
					case ExpressionValueRelation.Like:
						...
						switch (partForShow)
						{
							case EnumPart.AttachStringAndText:
								forShowValue = AttachDataAttribute.GetAttachedData<U>(values[i].ToString(), EnumAttachDataKey.String);
								break;
							case EnumPart.AttachInt:
								forShowValue = AttachDataAttribute.GetAttachedData<U>(values[i].ToString(), EnumAttachDataKey.Int);
								break;
							case EnumPart.AttachDecimal:
								forShowValue = AttachDataAttribute.GetAttachedData<U>(values[i].ToString(), EnumAttachDataKey.Decimal);
								break;
							case EnumPart.Text:
						...
						if (forShowValue.Contains(value.ToString().Trim()))
						{
							...
							{
								case EnumPart.AttachStringAndText:
									forSaveValue = AttachDataAttribute.GetAttachedData<U>(values[i].ToString(), EnumAttachDataKey.String);
								...
								case EnumPart.AttachDecimal:
									forSaveValue = AttachDataAttribute.GetAttachedData<U>(values[i].ToString(), EnumAttachDataKey.Decimal);
									addMethod.Invoke(list, new object[] { (decimal)(object)forSaveValue });
									break;

								case EnumPart.AttachInt:
									forSaveValue = AttachDataAttribute.GetAttachedData<U>(values[i].ToString(), EnumAttachDataKey.Int);
			...
			valueRelation = valueRelation == ExpressionValueRelation.Like ? ExpressionValueRelation.Contains : valueRelation;
			return ComposeCondition(member, list, valueRelation, expressionRelation);
		}

		public ExpressionCondition<T> And<U>(Expression<Func<T, object>> member, object value, ExpressionValueRelation valueRelation, EnumPart partForShow,EnumPart partForSave)
		{
		...
		/// <returns></returns>
		public ExpressionCondition<T> Or<U>(Expression<Func<T, object>> member, object value, ExpressionValueRelation valueRelation, EnumPart partForShow, EnumPart partForSave)
		{
		...
		public Expression<Func<T, bool>> ConditionExpression
		{
			get
			{
				...
				if (Criteria != null)
					invocationExpression = Expression.Invoke(Criteria, parameter);
				...
				if (conditionExpression == null && Criteria == null)
					...
					return Expression.Lambda<Func<T, bool>>(invocationExpression, parameter);
				else if (conditionExpression != null && Criteria != null)
				{
