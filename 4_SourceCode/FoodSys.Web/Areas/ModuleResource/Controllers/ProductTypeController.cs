...
namespace FoodSys.Web.Areas.ModuleResource.Controllers
{
	public class ProductTypeController : BaseController
	{
		...
		public ActionResult Index(ModelProductTypeIndex model, ExportHelper export)
		{
		...
		/// 创建 修改 查看  
		/// </summary>
		public ActionResult Create(ModelProductTypeCreate model)
		{
		...
		/// </summary>
		[HttpPost]
		public ActionResult Create(ModelProductTypeCreate model, FormCollection collection)
		{
			try
			{
				model.Save();
		...
		public ActionResult Delete(Guid id)
		{
			...
			try
			{
				...
				bizUTProductType.DeleteByID(id, NHibernate.NHibernateUtil.Guid);
				jsresult.Data = new { result = string.Empty };
			...
			try
			{
				for (int i = 0; i < checkboxItem.Length; i++)
				{
					if (checkboxItem[i].ToLower() == EnumTrueOrFalse.False.ToString().ToLower())
						continue;
					bizUTProductType.DeleteByID(checkboxItem[i], NHibernate.NHibernateUtil.String);
				}
